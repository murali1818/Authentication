...
exports.registerUser = async (req, res) => {
    try {
        ...
        if(password.length<8){
            return res.status(400).json({ success: false, message: "Password contain atlest 8 letters" });
        } 
        ...
        res.cookie('token', token, options).status(201).json({ success: true,message:"Registered Successfully!", token });
    } catch (err) {
        console.log(err)
        res.status(400).json({ success: false,message: err._message });
    }
};
//login
exports.login = async (req, res) => {
    try {
        ...
        res.cookie('token', token, options).status(201).json({ success: true,message:"Login Successfully!",token});
    } catch (err) {
        res.status(500).json({ success: false, message: err._message });
    }
};
//logout
